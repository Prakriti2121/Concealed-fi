...
import { TableSelector } from "../../../../admin/components/table-selector";
...
const EditPost = () => {
    ...
    const fetchPost = async () => {
      try {
        ...
        if (data.featuredImage) {
          setPreviewImage(data.featuredImage);
        }
  ...
  const addTag = async () => {
    ...
    if (existingTag) {
      ...
      // If tag doesn't exist, create it in the database
      try {
        const response = await fetch("/api/posts/tags", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  const handleSaveChanges = async (e: FormEvent) => {
    ...
    try {
      ...
      router.push("/admin/posts/all-posts");
    } catch (error) {
  ...
  return (
    <form onSubmit={handleSaveChanges} className="p-4 md:p-8 max-w-6xl mx-auto">
      <Card className="mb-8">
        <CardHeader>
                ...
                <div className="space-y-2 p-4">
                  <div className="flex items-center space-x-2">
                    <Checkbox
                      ...
                      onCheckedChange={(checked) => {
                        setIsUncategorized(checked === true);
                        if (checked) {
                          setSelectedCategory(null);
                        }
                      }}
                  ...
                  <div className="mt-4">
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent className="w-[200px] max-h-[300px] overflow-auto p-2 border-2">
                        <Input
                            ...
                            {filteredCategories.length === 0 ? (
                              ...
                              filteredCategories.map((category) => (
                                <DropdownMenuItem
                                  ...
                                  className="cursor-pointer rounded-md hover:bg-muted py-2"
                                >
                                  {category.name}
                                </DropdownMenuItem>
                              ))
                  ...
                  {selectedCategory && (
                    <div className="mt-4">
                      <Label className="mb-2 block">Selected Category:</Label>
                    ...
                    <DropdownMenu>
                      <DropdownMenuTrigger asChild>
                        <Button
                          ...
                          className="w-full justify-between border-2"
                        >
                          {newTagInput || "Select or add tags"}
                          <ChevronsUpDown className="ml-2 h-4 w-4 shrink-0 opacity-50" />
                        </Button>
                      </DropdownMenuTrigger>
                      <DropdownMenuContent className="w-[200px] max-h-[300px] overflow-auto p-2 border-2">
